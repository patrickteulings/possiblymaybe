/* global possiblymaybeGetHexLum, jQuery */
( function() {
	// Add listener for the "background_color" control.
	wp.customize( 'background_color', function( value ) {
		value.bind( function( to ) {
			var lum = possiblymaybeGetHexLum( to ),
				isDark = 127 > lum,
				...
				stylesheetID = 'possiblymaybe-customizer-inline-styles',
				stylesheet,
				styles;
			...
			if ( ! stylesheet.length ) {
				jQuery( '#possiblymaybe-style-inline-css' ).after( '<style id="' + stylesheetID + '"></style>' );
				stylesheet = jQuery( '#' + stylesheetID );
			}
