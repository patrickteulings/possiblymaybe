/* global possiblymaybeIsDarkMode, setTimeout */

// Check the color scheme preference and inject the classes if necessary.
if ( document.body.classList.contains( 'possiblymaybe-supports-dark-theme' ) ) {
	possiblymaybeDarkModeEditorInit();
}
 ...
 * @return {void}
 */
function possiblymaybeDarkModeEditorInit( attempt ) {
	...
	if ( possiblymaybeIsDarkMode() ) {
		if ( null === container ) {
			// Try again.
			if ( attempt < maxAttempts ) {
				setTimeout(
					function() {
						possiblymaybeDarkModeEditorInit( attempt + 1 );
					},
